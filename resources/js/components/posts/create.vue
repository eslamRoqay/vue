<template>
    <form @submit.prevent="storePost(post)" class="form-row" >
        <div class="form-group col-4 ">
            ...
            <input v-model="post.title" type="text" id="post_title" class="form-control" name="title"  placeholder="please enter title">
            <small  v-for="message in ValidationErrors?.title" class="form-text text-danger">{{message}}</small>

        </div>
        <div class="form-group col-4">
            <label for="post_content">content</label>
            <input v-model="post.content" type="text" id="post_content" class="form-control" name="content" placeholder="content">
        ...
        <div class="form-group col-4">
            ...
            <input v-model="post.image" type="file" id="post_image" class="form-control" name="image" >
            <small  v-for="message in ValidationErrors?.category_id" class="form-text text-danger">{{message}}</small>
        </div>

        <div class="col text-center">
        <button type="submit" class="btn btn-primary text-dark justify-center ">Submit</button>
        </div>
    </form>
</template>
<script>
