<template>
    <form @submit.prevent="updatePost(post)" class="form-row">
        ...
        <div class="form-group col-3">
            ...
            <input @change="post.image = $event.target.files[0]" type="file" id="post_image" class="form-control"
                   name="image">
            <small v-for="message in ValidationErrors?.image" class="form-text text-danger">{{ message }}</small>
        </div>

        <div class="col text-center">
            <button type="submit" :disabled='isDisable' class="btn btn-primary text-dark justify-center ">Submit
            </button>
        </div>
    </form>
</template>
...
export default {
    setup() {
        ...
        const {  getPost, post,updatePost, ValidationErrors, isDisable} = usePosts()
        const route = useRoute()
        onMounted(() => {
            getPost(route.params.id)
            getCategories()
        })
        return {categories, post, updatePost, ValidationErrors, isDisable}
    },
